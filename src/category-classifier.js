...
const BUSINESS_DYNAMICS_KEYWORDS = [
  ...
  '战略', '项目', '组织混乱', '四面楚歌', '员工', '高管', 'ceo', '董事会', '离职', '不信任',
  '内斗', '管理层', '动荡'
];
...
export function classifyNewsCategory(item = {}) {
  ...
  const strongPolicySignal = titlePolicyHits >= 1 || policyHits >= 2;

  if (strongPolicySignal) {
    return '政策与监管';
  }
  ...
  if (businessDynamicsHits >= 1 && policyHits === 0) {
    return '技术与研究';
  }

  if (policyHits >= 1 && businessDynamicsHits === 0) {
    return '政策与监管';
  }
